...
const getcart = async (req, res) => {
    try {
        var data = await cart.find({ user_id: req.body.user_id });
        var arr = [];
        data.forEach(element => {
            arr.push(element.product_id)
        });
        // console.log(arr);
        var data = await products.find({ _id: arr });
        // console.log(data);
        res.status(200).send({
            status: "success",
            data
...
const removecart = async (req, res) => {
    try {
        var data = await cart.deleteOne(req.body);
        res.status(200).send({
            status: "success",
            data
        })
...
module.exports = { createProduct, menwatch, womenwatch, gethomeproduct, getproduct, getsingleproduct, userSelectProduct, updateproduct, deleteproduct, addtocart, getcart,removecart };